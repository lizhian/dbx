export default {
  ...
  fileManager: {
    title: "File Manager",
    add: "New file connection",
    emptyConnections: "No file connections",
    emptyDirectory: "The root directory is empty",
    name: "Name",
    ...
    virtualHostStyle: "Use virtual-hosted-style requests",
    anonymous: "Use anonymous access",
    clearS3Credentials: "Remove saved S3 credentials",
    root: "Root",
    username: "Username",
    password: "Password",
    keepPassword: "Leave blank to keep the saved password",
    clearPassword: "Remove saved password",
    ftpSecurity: "FTP is unencrypted. Usernames, passwords, and file contents travel over the network in plaintext.",
    s3Security: "S3 credentials are stored separately from connection configuration. Use an HTTPS endpoint to encrypt credentials and file contents in transit.",
    webdavSecurity: "WebDAV credentials are stored separately from connection configuration. Use HTTPS because HTTP exposes credentials and file contents.",
    sftpSecurity: "SFTP verifies host keys strictly. Authentication uses your default SSH config, SSH agent, or inline private-key material stored separately from connection configuration. Password-only SFTP is unsupported.",
    sftpUnsupported: "Unsupported on this platform",
    hdfsSecurity: "HDFS Native uses a pure Rust client with simple authentication and one explicit NameNode. Kerberos, HA, ViewFS, and multiple NameNodes are rejected.",
    webhdfsSecurity: "WebHDFS delegation tokens are stored separately. DataNode redirects are restricted to exact allowlisted origins; use HTTPS and a trusted CA.",
    implementation: "Implementation",
    native: "Native",
    nameNodeUri: "NameNode URI",
    hadoopConfigDirectory: "Hadoop config directory",
    hdfsOptions: "Allowed Hadoop options",
    hdfsOptionsHint: "One key=value per line",
    hdfsUseUserEnvironment: "Use the existing HADOOP_USER_NAME environment value",
    webhdfsCopyRisk: "WebHDFS copy relays bytes through DBX in bounded chunks, verifies the operation-owned partial, and publishes it with REST RENAME. Existing destinations are never deleted for replacement.",
    webhdfsRenameRisk: "WebHDFS rename uses REST RENAME with best-effort no-clobber preflight. Existing destinations cannot be safely replaced.",
    webhdfsUploadRiskConfirm: 'Upload to "{path}" using a bounded WebHDFS stream and publish with REST RENAME?',
    webhdfsSimple: "Simple user",
    webhdfsDelegation: "Delegation token",
    webhdfsDelegationToken: "Delegation token",
    clearWebhdfsCredentials: "Remove saved delegation token",
    webhdfsDataNodeOrigins: "Allowed DataNode origins (one per line)",
    webhdfsHostnameMapping: "DataNode hostname mapping (host=address)",
    webhdfsTlsCa: "TLS CA certificate path",
    webhdfsProxy: "Proxy URL",
    webhdfsProxyBypass: "Proxy bypass list",
    webhdfsConnectTimeout: "Connect timeout (seconds)",
    webhdfsControlTimeout: "Control timeout (seconds)",
    webhdfsIdleTimeout: "Stream idle timeout (seconds)",
    webhdfsChunkSize: "Relay chunk size (MiB)",
    webhdfsPermission: "Create permission",
    webhdfsReplication: "Replication",
    webhdfsBlockSize: "Block size (bytes)",
    webhdfsBufferSize: "Buffer size (bytes)",
    webhdfsDisableListBatch: "Disable batched directory listing",
    sshConfig: "Default SSH config",
    sshAgent: "SSH agent",
    sshPrivateKey: "Inline private key",
    privateKeyMaterial: "Private-key material",
    privateKeyPassphrase: "Private-key passphrase",
    keepPrivateKey: "Leave blank to keep the saved private key and passphrase",
    clearSftpCredentials: "Remove saved SFTP private-key credentials",
    authentication: "Authentication",
    authNone: "None",
    authBasic: "Basic username and password",
    authBearer: "Bearer token",
    bearerToken: "Bearer token",
    clearWebdavCredentials: "Remove saved WebDAV credentials",
    test: "Test connection",
    edit: "Edit connection",
    delete: "Delete connection",
    deleteConfirm: "Delete this file connection and its saved credentials?",
    createDirectory: "Create directory",
    directoryPath: "Directory path",
    directoryPathHint: "Relative to the configured root, for example reports/2026",
    deleteEntry: "Delete",
    deleteEntryConfirm: 'Delete "{name}"? Only files and empty directories can be deleted.',
    operationComplete: "File operation completed",
    loadError: "Failed to load file connections",
    testSuccess: "Connection test passed",
    refresh: "Refresh",
    stageConfiguration: "Configuration",
    stageAuthentication: "Authentication",
    stageHostKey: "Host key",
    stageNameNode: "NameNode",
    stageNameNodeRpc: "NameNode RPC",
    stageDataNode: "DataNode",
    stageDataNodeWrite: "DataNode write",
    stageDataNodeRead: "DataNode read",
    type: "Type",
    size: "Size",
    modified: "Modified",
    loadMore: "Load more",
    cursorExpired: "This directory view expired. Refresh it before continuing.",
    metadata: "Metadata",
    contentType: "Content type",
    ...
    download: "Download",
    upload: "Upload file",
    uploadRiskConfirm: 'Upload to "{path}" using FTP best-effort no-clobber? FTP cannot make the final existence check and rename atomic, so an external writer can race this upload. DBX will not intentionally replace a destination it observes.',
    s3UploadRiskConfirm: 'Upload to "{path}"? S3 publish uses a bounded multipart upload followed by server-side copy. No-clobber is best effort unless the service honors conditional copy.',
    copy: "Copy",
    rename: "Rename",
    destinationPath: "Destination path",
    copyRenameRisk: "FTP uses best-effort no-clobber. The final existence check and publish are not atomic, so an external writer can still race this operation.",
    s3CopyRisk: "S3 copy uses server-side CopyObject. No file bytes are relayed through DBX. No-clobber remains best effort on services that do not honor conditional copy.",
    s3RenameRisk: "S3 rename is a non-atomic server-side copy followed by source deletion. A durable partial result is kept if source deletion fails.",
    webdavCopyRisk: "WebDAV COPY runs on the server and does not relay file bytes through DBX. No-clobber is best effort because this backend always sends Overwrite: T.",
    webdavRenameRisk: "WebDAV rename uses native server-side MOVE. No-clobber is best effort and an external writer can race the destination preflight.",
    sftpUploadRiskConfirm: 'Upload to "{path}"? SFTP publishes the operation-owned partial with a native rename. No-clobber remains best effort because an external writer can race the destination preflight.',
    sftpCopyRisk: "SFTP copy relays file bytes through DBX using bounded memory, then publishes with native rename. No-clobber is best effort.",
    sftpRenameRisk: "SFTP rename uses native server-side rename. No-clobber is best effort and an external writer can race the destination preflight.",
    hdfsUploadRiskConfirm: 'Upload to "{path}"? HDFS publishes the operation-owned partial with native rename2. No-clobber remains best effort because an external writer can race the destination preflight.',
    hdfsCopyRisk: "HDFS copy relays file bytes through DBX in bounded application chunks of up to 4 MiB, then publishes with native rename2. No-clobber remains best effort.",
    hdfsRenameRisk: "HDFS rename uses native Hadoop rename2. Replacing an existing destination requires explicit confirmation; no-clobber remains best effort at the product boundary.",
    replaceDestination: "Replace the destination if it exists",
    replaceConfirm: 'Replace "{path}"? This explicitly allows existing remote data to be overwritten.',
    retrySourceDelete: "Retry source deletion after fingerprint checks",
    operationOutcome: "Outcome",
    transfers: "Transfers",
    noTransfers: "No file transfers",
    cancelTransfer: "Cancel transfer",
    transferQueued: "Queued",
    transferRunning: "Downloading",
    transferUploading: "Uploading",
    transferCopying: "Copying",
    transferRenaming: "Renaming",
    transferCancelling: "Cancelling",
    transferPublishing: "Finalizing",
    transferCompleted: "Completed",
    transferFailed: "Failed",
    transferCancelled: "Cancelled",
    transferPartial: "Partial destination",
    partialDestination: "Partial",
    abortOutcome: "Abort / cleanup",
