...
const regularSurfaceCount = computed(() => regularTabs.value.length + (props.driverStoreOpen ? 1 : 0) + (props.fileManagerOpen ? 1 : 0) + (props.settingsPageOpen ? 1 : 0));
...
type SpecialRegularSurface = "driverStore" | "fileManager" | "settings";
...
function getSpecialRegularTabMenuItems(surface: SpecialRegularSurface): ContextMenuItem[] {
  const keep = surface;
  const closeCurrent = surface === "driverStore" ? () => emit("close-driver-store") : surface === "fileManager" ? () => emit("close-file-manager") : () => emit("close-settings-page");
...
watch(
  () => props.fileManagerActive,
  (show) => {
    if (!show) return;
    nextTick(() => {
      if (isWrapLayout.value) return;
      const container = tabsContainerRef.value;
      const el = container?.querySelector("[data-file-manager-tab]");
...
function tabColorStyle(tab: QueryTab) {
  ...
  const isActive = tab.id === queryStore.activeTabId && !props.driverStoreActive && !props.fileManagerActive && !props.settingsPageActive;
...
<template>
  <div v-if="queryStore.tabs.length > 0 || driverStoreOpen || fileManagerOpen || settingsPageOpen" class="app-tab-bar relative flex w-full min-w-0 shrink-0 overflow-hidden" :class="tabBarClass">
                    ...
                    :class="
                      isClassicLayout
                        ? [
                            ...
                            tab.id === queryStore.activeTabId && !driverStoreActive && !fileManagerActive && !settingsPageActive ? 'bg-background text-foreground' : 'text-foreground/70 hover:text-foreground/90',
                          ]
                        : [
                            compactTabTitle ? 'min-w-24' : 'min-w-38',
                            'h-7 rounded-md border',
                            tab.id === queryStore.activeTabId && !driverStoreActive && !fileManagerActive && !settingsPageActive ? 'text-foreground font-medium' : 'border-border/60 text-foreground/70 hover:border-border hover:text-foreground/90',
                          ]
                    "
                    :style="[tabColorStyle(tab), tabDropStyle(tab.id)]"
                    :data-active-tab="tab.id === queryStore.activeTabId && !driverStoreActive && !fileManagerActive && !settingsPageActive"
          ...
          <CustomContextMenu v-if="fileManagerOpen" :items="getSpecialRegularTabMenuItems('fileManager')" v-slot="{ onContextMenu }">
            <div :class="isClassicLayout ? 'h-full' : ''" @contextmenu="onContextMenu">
              <div
                ...
                role="tab"
                tabindex="0"
                :aria-selected="fileManagerActive"
                @click="emit('activate-file-manager')"
                @keydown.enter.self.prevent="emit('activate-file-manager')"
                @keydown.space.self.prevent="emit('activate-file-manager')"
              ...
              >
                ...
                <button :aria-label="t('common.close')" class="rounded hover:bg-muted-foreground/20 p-0.5 shrink-0" @click.stop="emit('close-file-manager')">
                  <X class="h-3 w-3" />
                </button>
              </div>
            </div>
          ...
          <PopoverContent align="end" class="w-auto min-w-56 max-w-80 max-h-[min(70vh,28rem)] gap-0 overflow-y-auto rounded-[6px] p-1" @click.stop @keydown.stop>
            <CustomContextMenu v-for="tab in queryStore.tabs" :key="tab.id" :items="getTabMenuItems(tab)" v-slot="{ onContextMenu }">
              <div
                ...
                :class="tab.id === queryStore.activeTabId && !driverStoreActive && !fileManagerActive && !settingsPageActive ? 'bg-accent/70 text-accent-foreground' : ''"
