...
<template>
  <Popover>
    ...
    <PopoverContent data-file-download-list align="end" class="w-[min(92vw,38rem)] gap-0 overflow-hidden p-0" :side-offset="8">
      ...
      <div v-else class="max-h-96 overflow-auto">
        <div v-for="task in visibleTasks" :key="task.id" :data-file-download-task="task.remotePath" class="flex min-w-[30rem] items-center gap-2 border-b px-4 py-3 last:border-b-0">
          <div class="flex min-w-0 items-center gap-2">
            ...
            <span class="w-40 truncate text-sm font-medium" :title="task.remotePath">{{ task.fileName }}</span>
          </div>
          <span class="w-28 shrink-0 truncate text-xs text-muted-foreground" :class="{ 'text-destructive': task.status === 'failed' }" :title="progressText(task)">{{ progressText(task) }}</span>
          <span class="w-24 shrink-0 truncate text-[11px] tabular-nums text-muted-foreground" :title="sizeText(task)">{{ sizeText(task) }}</span>
