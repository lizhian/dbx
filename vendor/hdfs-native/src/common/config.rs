...
impl Configuration {
    pub(crate) fn new(conf_dir: Option<String>, conf_map: Option<HashMap<String, String>>) -> Result<Self> {
    ...
    fn get_boolean(&self, key: &str, default: bool) -> bool {
        self.get(key).map(|v| v.to_lowercase() == "true").unwrap_or(default)
    }

    pub(crate) fn security_enabled(&self) -> bool {
        self.get(HADOOP_SECURITY_AUTHENTICATION).is_some_and(|c| c != "simple")
    }
    ...
    pub(crate) fn datanode_hostname_enabled(&self) -> bool {
    ...
    pub(crate) fn get_urls_for_nameservice(&self, nameservice: &str) -> Result<Vec<String>> {
        let urls: Vec<String> = self
            ...
            .flat_map(|namenodes| {
                namenodes.split(',').flat_map(|namenode_id| {
                    self.map
                        .get(&format!("{HA_NAMENODE_RPC_ADDRESS_PREFIX}.{nameservice}.{namenode_id}"))
                        .map(|s| s.to_string())
                })
            })
            .collect();

        let mut urls = if self.get_boolean(&format!("{DFS_CLIENT_FAILOVER_RESOLVE_NEEDED}.{nameservice}"), false) {
            let use_fqdn = self.get_boolean(&format!("{DFS_CLIENT_FAILOVER_RESOLVER_USE_FQDN}.{nameservice}"), true);
            ...
            debug!("Namenodes for {} resolved to {:?}", nameservice, resolved_urls);
        ...
        if self.get_boolean(&format!("{DFS_CLIENT_FAILOVER_RANDOM_ORDER}.{nameservice}"), false) {
    ...
    pub(crate) fn get_proxy_for_nameservice(&self, nameservice: &str) -> Option<&str> {
        self.get(&format!("{DFS_CLIENT_FAILOVER_PROXY_PROVIDER}.{nameservice}"))
    }
    ...
    pub(crate) fn get_mount_table(&self, cluster: &str) -> Vec<(Option<String>, String)> {
        self.map
            .iter()
            .flat_map(|(key, value)| {
                if let Some(path) = key.strip_prefix(&format!("{VIEWFS_MOUNTTABLE_PREFIX}.{cluster}.link.")) {
    ...
    pub fn get_replace_datanode_on_failure_policy(&self) -> crate::hdfs::replace_datanode::ReplaceDatanodeOnFailure {
        use crate::hdfs::replace_datanode::{Policy, ReplaceDatanodeOnFailure};

        let enabled = self.get_boolean(DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY, true);
        if !enabled {
            return ReplaceDatanodeOnFailure::new(Policy::Disable, false);
        }

        let best_effort = self.get_boolean(DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_BEST_EFFORT_KEY, false);

        let policy_str =
            self.get(DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY).unwrap_or("DEFAULT").to_uppercase();
    ...
    fn read_from_file(path: &Path) -> Result<Vec<(String, String)>> {
        ...
        let pairs = tree
            ...
            .into_iter()
            .flat_map(|config| config.children().filter(|c| c.tag_name().name() == "property"))
            .flat_map(|property| {
    ...
    fn parse_conf(conf_dir: PathBuf) -> Result<HashMap<String, String>> {
        ...
        for file in ["core-site.xml", "hdfs-site.xml"] {
            ...
            if config_path.as_path().exists() {
                Self::read_from_file(config_path.as_path())?.into_iter().for_each(|(key, value)| {
...
impl EntityResolver {
    ...
    fn resolve<'a>(&'a self, uri: &str) -> core::result::Result<Option<&'a str>, String> {
        ...
        let entity_file_metadata = match fs::metadata(&full_path) {
            Ok(m) => m,
            Err(e) => {
                return Err(format!("failed to get metadata of entity file {}: {}", full_path.display(), e));
            }
        };
        ...
        if entity_file_size > self.file_length_limit {
            return Err(format!("entity file {} is too large ({} bytes)", full_path.display(), entity_file_size));
        }
        ...
        let entity_file_content = fs::read_to_string(&full_path)
            .map_err(|e| format!("read entity file content (path {}): {}", full_path.display(), e))?;
    ...
    fn resolve_full_path(&self, uri: &str) -> core::result::Result<PathBuf, String> {
        ...
        for c in path.components() {
            match c {
                Component::CurDir => {}
                Component::ParentDir => {
                    return Err("parent directory components `..` are not allowed in entity URIs".to_string());
                }
                ...
                Component::Prefix(p) => {
                    return Err(format!("absolute paths (with prefix {p:?}) are not allowed in entity URIs"));
                }
            }
        }
...
mod test {
    ...
    use super::{
        ...
        DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY, EntityResolver, HA_NAMENODE_RPC_ADDRESS_PREFIX,
        HA_NAMENODES_PREFIX, VIEWFS_MOUNTTABLE_PREFIX,
    };

    #[test]
    fn datanode_hostname_option_is_explicit_and_defaults_to_ip() {
        let default = Configuration::new(None, None).unwrap();
        assert!(!default.datanode_hostname_enabled());
        let hostname = Configuration::new(
            None,
            Some(HashMap::from([("dfs.client.use.datanode.hostname".to_string(), "true".to_string())])),
        )
        .unwrap();
        assert!(hostname.datanode_hostname_enabled());
    }

    #[test]
    fn test_mount_table_config() {
        let mounts =
            [("clusterX", "/view1", "/hdfs1"), ("clusterX", "/view2", "/hdfs2"), ("clusterY", "/view3", "/hdfs3")];
        ...
        assert_eq!(
            vec![
                ...
                (Some("/view1".to_string()), "hdfs://127.0.0.1:9000/hdfs1".to_string()),
                (Some("/view2".to_string()), "hdfs://127.0.0.1:9000/hdfs2".to_string())
            ],
            mount_table
        );
        ...
        assert_eq!(
            ...
            vec![
                ...
                (Some("/view3".to_string()), "hdfs://127.0.0.1:9000/hdfs3".to_string())
            ]
        );
    }
    ...
    fn test_namenode_resolving() {
        let mut config = Configuration {
            map: vec![
                (format!("{}.{}", HA_NAMENODES_PREFIX, "test"), "namenode".to_string()),
                (format!("{}.{}.{}", HA_NAMENODE_RPC_ADDRESS_PREFIX, "test", "namenode"), "localhost:9000".to_string()),
                (format!("{}.{}", DFS_CLIENT_FAILOVER_RESOLVE_NEEDED, "test"), "true".to_string()),
            ]
            .into_iter()
            .collect(),
        };
        ...
        config.map.insert(format!("{}.{}", DFS_CLIENT_FAILOVER_RESOLVER_USE_FQDN, "test"), "false".to_string());
    ...
    fn test_replace_datanode_policy_config() {
        // Test default policy
        let config = Configuration { map: HashMap::new() };
        let policy = config.get_replace_datanode_on_failure_policy();
        assert!(!policy.is_best_effort());

        // Test disabled policy
        let config = Configuration {
            map: [(DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "false".to_string())]
                .into_iter()
                .collect(),
        };
        let policy = config.get_replace_datanode_on_failure_policy();
        ...
        let config = Configuration {
            map: [
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "true".to_string()),
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY.to_string(), "NEVER".to_string()),
            ]
            .into_iter()
            .collect(),
        };
        ...
        let config = Configuration {
            map: [
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "true".to_string()),
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY.to_string(), "ALWAYS".to_string()),
            ]
            .into_iter()
            .collect(),
        };
        ...
        let config = Configuration {
            map: [
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "true".to_string()),
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_BEST_EFFORT_KEY.to_string(), "false".to_string()),
            ]
            .into_iter()
            .collect(),
        };
        ...
        let config = Configuration {
            map: [
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "true".to_string()),
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_BEST_EFFORT_KEY.to_string(), "true".to_string()),
            ]
            .into_iter()
            .collect(),
        };
        ...
        let config = Configuration {
            map: [
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY.to_string(), "true".to_string()),
                (DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_BEST_EFFORT_KEY.to_string(), "invalid".to_string()),
            ]
            .into_iter()
            .collect(),
        };
    ...
    fn test_resolve_full_path() {
        ...
        assert_eq!(result.unwrap_err(), "parent directory components `..` are not allowed in entity URIs");
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "absolute paths are not allowed in entity URIs");
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), basepath.join("dir1").join("dir2").join("dir3").join("config.xml"));
    }
    ...
    fn test_config_read_from_file() {
        ...
        "# };
        fs::write(example_entity_path, example_entity_content).expect("Failed to write entity file");

        // Get the config map from files.
        let map: HashMap<String, String> = {
            let pairs = Configuration::read_from_file(&config_path).expect("Failed to read config file");
        ...
        assert_eq!(map.get("fs.defaultFS").map(|s| s.as_str()), Some("hdfs://localhost:9000"));
        assert_eq!(map.get("dfs.replication").map(|s| s.as_str()), Some("3"));
        assert_eq!(map.get("custom.property").map(|s| s.as_str()), Some("entity-value"));
    }
}
