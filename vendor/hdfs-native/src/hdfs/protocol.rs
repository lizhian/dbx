...
use crate::proto::hdfs::{self, DataEncryptionKeyProto, FsServerDefaultsProto, GetDataEncryptionKeyResponseProto};
...
impl NamenodeProtocol {
    ...
    async fn call<Req, Res>(&self, method_name: &'static str, message: Req, write: bool) -> Result<Res>
    where
        ...
        Res: Message + Default + std::fmt::Debug,
    {
        ...
        let response = self.proxy.call(method_name, &message.encode_length_delimited_to_vec(), write).await?;
    ...
    pub(crate) async fn get_file_info(&self, src: &str) -> Result<hdfs::GetFileInfoResponseProto> {
        let message = hdfs::GetFileInfoRequestProto { src: src.to_string() };
    ...
    pub(crate) async fn get_listing(
        ...
        let message = hdfs::GetListingRequestProto { src: src.to_string(), start_after, need_location };
    ...
    pub(crate) async fn get_block_locations(
        ...
        let message = hdfs::GetBlockLocationsRequestProto { src: src.to_string(), offset, length };
    ...
    pub(crate) async fn get_data_encryption_key(&self) -> Result<GetDataEncryptionKeyResponseProto> {
        let message = hdfs::GetDataEncryptionKeyRequestProto::default();

        self.call("getDataEncryptionKey", message, false).await
    }

    pub(crate) async fn get_cached_data_encryption_key(&self) -> Result<Option<DataEncryptionKeyProto>> {
    ...
    pub(crate) async fn append(&self, src: &str, new_block: bool) -> Result<hdfs::AppendResponseProto> {
        ...
        let message =
            hdfs::AppendRequestProto { src: src.to_string(), client_name: self.client_name.clone(), flag: Some(flag) };
    ...
    pub(crate) async fn update_block_for_pipeline(
        ...
        let message = hdfs::UpdateBlockForPipelineRequestProto { block, client_name: self.client_name.clone() };
    ...
    pub(crate) async fn complete(
        ...
        let message =
            hdfs::CompleteRequestProto { src: src.to_string(), client_name: self.client_name.clone(), last, file_id };
    ...
    pub(crate) async fn mkdirs(
        ...
        let message = hdfs::MkdirsRequestProto { src: src.to_string(), masked, create_parent, ..Default::default() };
        self.call("mkdirs", message, true).await
    }

    pub(crate) async fn rename(&self, src: &str, dst: &str, overwrite: bool) -> Result<hdfs::Rename2ResponseProto> {
    ...
    pub(crate) async fn delete(&self, src: &str, recursive: bool) -> Result<hdfs::DeleteResponseProto> {
        let message = hdfs::DeleteRequestProto { src: src.to_string(), recursive };
        self.call("delete", message, true).await
    }

    pub(crate) async fn renew_lease(&self, namespaces: Vec<String>) -> Result<hdfs::RenewLeaseResponseProto> {
        let message = hdfs::RenewLeaseRequestProto { client_name: self.client_name.clone(), namespaces };
        self.call("renewLease", message, true).await
    }

    pub(crate) async fn set_times(&self, src: &str, mtime: u64, atime: u64) -> Result<hdfs::SetTimesResponseProto> {
        let message = hdfs::SetTimesRequestProto { src: src.to_string(), mtime, atime };
    ...
    pub(crate) async fn set_permission(&self, src: &str, permission: u32) -> Result<hdfs::SetPermissionResponseProto> {
    ...
    pub(crate) async fn set_replication(
        ...
        let message = hdfs::SetReplicationRequestProto { src: src.to_string(), replication };
        self.call("setReplication", message, true).await
    }

    pub(crate) async fn get_content_summary(&self, path: &str) -> Result<hdfs::GetContentSummaryResponseProto> {
        let message = hdfs::GetContentSummaryRequestProto { path: path.to_string() };
    ...
    pub(crate) async fn modify_acl_entries(
        ...
        let message =
            hdfs::ModifyAclEntriesRequestProto { src: path.to_string(), acl_spec: acl_spec.into_iter().collect() };
    ...
    pub(crate) async fn remove_acl_entries(
        ...
        let message =
            hdfs::RemoveAclEntriesRequestProto { src: path.to_string(), acl_spec: acl_spec.into_iter().collect() };

        self.call("removeAclEntries", message, true).await
    }

    pub(crate) async fn remove_default_acl(&self, path: &str) -> Result<hdfs::RemoveDefaultAclResponseProto> {
        let message = hdfs::RemoveDefaultAclRequestProto { src: path.to_string() };
    ...
    pub(crate) async fn remove_acl(&self, path: &str) -> Result<hdfs::RemoveAclResponseProto> {
        let message = hdfs::RemoveAclRequestProto { src: path.to_string() };
        self.call("removeAcl", message, true).await
    }

    pub(crate) async fn set_acl(&self, path: &str, acl_spec: Vec<AclEntry>) -> Result<hdfs::SetAclResponseProto> {
        let message = hdfs::SetAclRequestProto { src: path.to_string(), acl_spec: acl_spec.into_iter().collect() };
        self.call("setAcl", message, true).await
    }

    pub(crate) async fn get_acl_status(&self, path: &str) -> Result<hdfs::GetAclStatusResponseProto> {
        let message = hdfs::GetAclStatusRequestProto { src: path.to_string() };
...
impl LeaseTracker for Arc<NamenodeProtocol> {
    fn add_file_lease(&self, file_id: u64, namespace: Option<String>) {
        self.open_files.lock().unwrap().insert(LeasedFile { file_id, namespace });

        let mut lease_renewer = self.lease_renewer.lock().unwrap();
        if lease_renewer.is_none() {
            *lease_renewer = Some(start_lease_renewal(Arc::downgrade(self), self.handle.clone()));
        }
    }

    fn remove_file_lease(&self, file_id: u64, namespace: Option<String>) {
        self.open_files.lock().unwrap().remove(&LeasedFile { file_id, namespace });
    }
}

fn start_lease_renewal(protocol: std::sync::Weak<NamenodeProtocol>, handle: tokio::runtime::Handle) -> JoinHandle<()> {
    handle.spawn(async move {
        ...
        loop {
            ...
            let (writing, namespaces) = {
                ...
                let namespaces: HashSet<String> = files.iter().flat_map(|f| f.namespace.as_ref()).cloned().collect();
            ...
            if !writing {
                ...
                if last_renewal.is_some_and(|last| {
                    last.elapsed().is_ok_and(|elapsed| elapsed.as_secs() > LEASE_RENEWAL_INTERVAL_SECS)
                }) {
...
mod lease_renewal_drop_tests {
    ...
    fn protocol() -> Arc<NamenodeProtocol> {
        let handle = Handle::current();
        let config = Arc::new(Configuration::new(None, None).unwrap());
        let proxy = NameServiceProxy::new(&Url::parse("hdfs://127.0.0.1:1").unwrap(), config, handle.clone()).unwrap();
        Arc::new(NamenodeProtocol::new(proxy, handle))
    }

    async fn assert_protocol_drops_after_starting_renewer(file_id: u64) {
        let protocol = protocol();
        protocol.add_file_lease(file_id, None);
        ...
        .await
        .expect("lease renewer retained NamenodeProtocol after its owner was dropped");
    }

    #[tokio::test]
    async fn renewer_does_not_create_a_protocol_ownership_cycle() {
        assert_protocol_drops_after_starting_renewer(1).await;
    }

    #[tokio::test]
    async fn a_later_protocol_can_start_and_release_its_own_renewer() {
        assert_protocol_drops_after_starting_renewer(1).await;
        assert_protocol_drops_after_starting_renewer(2).await;
    }
}
