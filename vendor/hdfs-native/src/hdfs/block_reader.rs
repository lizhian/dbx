...
use crate::{
    ...
    proto::{
        common,
        hdfs::{self, BlockOpResponseProto, LocatedBlockProto, PacketHeaderProto, ReadOpChecksumInfoProto},
    },
};
...
fn abort_join_handle<T>(handle: &mut Option<JoinHandle<T>>) {
    if let Some(handle) = handle.take() {
        handle.abort();
    }
}

async fn await_join_handle<T>(handle: &mut Option<JoinHandle<T>>) -> std::result::Result<T, tokio::task::JoinError> {
    let result = handle.as_mut().expect("listener is present until its await completes").await;
    handle.take();
    result
}
...
pub(crate) fn get_block_stream(
    ...
    if let Some(ec_schema) = ec_schema {
        StripedBlockStream::new(protocol, block, offset, len, ec_schema, config, handle).into_stream().boxed()
    } else {
        ReplicatedBlockStream::new(protocol, block, offset, len, config, handle).into_stream().boxed()
    }
}
...
async fn connect_and_send(
    ...
    if crate::test::DATANODE_CONNECT_FAULT_INJECTOR.swap(false, std::sync::atomic::Ordering::SeqCst) {
        return Err(HdfsError::DataTransferError("DataNode connect fault injection".to_string()));
    }
...
impl ReplicatedBlockStream {
    fn new(
        ...
        Self { protocol, block, offset, len, config, handle, listener: None, sender, receiver, next_replica: 0 }
    }

    async fn select_next_datanode(&mut self) -> Result<(DatanodeConnection, Option<ReadOpChecksumInfoProto>)> {
        loop {
            if self.next_replica >= self.block.locs.len() {
                return Err(HdfsError::DataTransferError(format!("All DataNodes failed: {:?}", self.block.locs)));
            }
            ...
            .await
            {
                Ok((connection, response)) => {
                    if response.status() != hdfs::Status::Success {
                        warn!("Read operation did not succeed for DataNode {:?}: {}", datanode, response.message());
                    } else {
    ...
    async fn next_packet_from_receiver(&mut self) -> Option<Result<(PacketHeaderProto, Bytes)>> {
        #[cfg(feature = "integration-test")]
        if crate::test::DATANODE_READ_FAULT_INJECTOR.swap(false, std::sync::atomic::Ordering::SeqCst) {
            return Some(Err(HdfsError::IOError(std::io::Error::from(std::io::ErrorKind::UnexpectedEof))));
        }
    ...
    async fn next_packet(&mut self) -> Result<Option<Bytes>> {
        ...
        let (header, data) = loop {
            ...
            let retry_connection = if self.listener.is_none() {
                ...
                self.listener =
                    Some(Self::start_packet_listener(connection, checksum_info, self.sender.clone(), &self.handle));
                false
            } else {
                true
            };
            ...
            match self.next_packet_from_receiver().await {
                ...
                Some(Err(e)) => {
                    ...
                    abort_join_handle(&mut self.listener);
                }
                None => {
                    // This means there's a disconnect between the data we are getting back and what we asked for,
                    // so just raise an error
                    return Err(HdfsError::DataTransferError("Not enough data returned from DataNode".to_string()));
                }
            }
        };
        ...
        if self.len == 0 {
            let conn = await_join_handle(&mut self.listener).await.unwrap()?;
            DATANODE_CACHE.release(conn);
        }

        Ok(Some(data.slice(packet_offset..(packet_offset + packet_len))))
    }
    ...
    fn start_packet_listener(
        ...
        handle.spawn(async move {
            loop {
                match Self::get_next_packet(&mut connection, checksum_info).await {
                    Ok((header, data)) => {
                        if data.is_empty() {
                        ...
                        if sender.send(Ok((header, data))).await.is_err() {
                    ...
                    Err(error) => {
                        ...
                        return Err(HdfsError::DataTransferError(
                            "DataNode packet listener stopped after a read failure".to_string(),
                        ));
                    }
                }
            }
...
impl Drop for ReplicatedBlockStream {
    fn drop(&mut self) {
        abort_join_handle(&mut self.listener);
    }
}
...
mod block_reader_drop_tests {
    ...
    #[tokio::test]
    async fn dropping_unconsumed_listener_aborts_its_task() {
        ...
        tokio::task::yield_now().await;
        abort_join_handle(&mut listener);
        assert!(listener.is_none());
        wait_for_drop(&dropped).await;
    }

    #[tokio::test]
    async fn cancelling_listener_await_keeps_handle_owned_for_abort() {
        let dropped = Arc::new(AtomicBool::new(false));
        let task_dropped = dropped.clone();
        let mut listener = Some(tokio::spawn(async move {
            let _marker = DropMarker(task_dropped);
        ...
        tokio::task::yield_now().await;
        {
            let await_listener = await_join_handle(&mut listener);
            tokio::pin!(await_listener);
            tokio::select! {
                _ = &mut await_listener => panic!("pending listener unexpectedly completed"),
...
impl CellReader {
    fn new(cell_size: usize, block_stream: Option<ReplicatedBlockStream>) -> Self {
        Self { cell_size, cell_buffer: BytesMut::with_capacity(cell_size), current_packet: Bytes::new(), block_stream }
    }
    ...
    async fn next_cell(&mut self) -> Result<Bytes> {
        ...
        while self.cell_buffer.len() < self.cell_size {
            ...
            let bytes_to_copy = usize::min(self.cell_size - self.cell_buffer.len(), self.current_packet.remaining());

            self.cell_buffer.put(self.current_packet.split_to(bytes_to_copy));
        }

        // Pad a partial final cell with zeros
        self.cell_buffer.resize(self.cell_size, 0);

        Ok(std::mem::replace(&mut self.cell_buffer, BytesMut::with_capacity(self.cell_size)).freeze())
    }
}
...
impl StripedBlockStream {
    fn new(
        ...
        let datanode_infos: Vec<(hdfs::DatanodeInfoProto, common::TokenProto)> =
            block.locs.iter().cloned().zip(block.block_tokens.iter().cloned()).collect();

        let block_map: HashMap<usize, (hdfs::DatanodeInfoProto, common::TokenProto)> =
            block.block_indices().iter().copied().map(|i| i as usize).zip(datanode_infos).collect();
        ...
        while good_cells < self.ec_schema.data_units {
            ...
            if let Some(reader) = self.cell_readers[block_index].as_mut() {
                match reader.next_cell().await {
                    ...
                    Err(e) => {
                        warn!("Error reading erasure coded block, trying next replica: {:?}", e);
    ...
    async fn start_next_reader(&mut self) -> Result<bool> {
        if self.cell_readers.len() >= self.ec_schema.data_units + self.ec_schema.parity_units {
            return Err(HdfsError::ErasureCodingError("Not enough valid shards".to_string()));
        }
        ...
        let max_block_offset = self.ec_schema.max_offset(index, self.block.b.num_bytes() as usize);
        ...
        if len == 0 {
            self.cell_readers.push(Some(CellReader::new(self.ec_schema.cell_size, None)));
            Ok(true)
            ...
            self.cell_readers.push(Some(CellReader::new(self.ec_schema.cell_size, Some(block_stream))));
    ...
    fn into_stream(self) -> impl Stream<Item = Result<Bytes>> {
        stream::unfold((self, VecDeque::new()), |(mut stream, mut buffers)| async move {
            if buffers.is_empty() {
