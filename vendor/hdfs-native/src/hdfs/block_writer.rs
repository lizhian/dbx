...
impl BlockWriter {
    ...
    pub(crate) async fn new(
        ...
        let block_writer = if let Some(ec_schema) = ec_schema {
            Self::Striped(StripedBlockWriter::new(protocol, block, ec_schema, server_defaults, config, handle, status))
        } else {
            Self::Replicated(
                ReplicatedBlockWriter::new(src, protocol, block, server_defaults, config, handle, status).await?,
            )
        };
        Ok(block_writer)
    }
...
impl Pipeline {
    fn new(connection: DatanodeConnection, handle: &Handle) -> Self {
        ...
        let (ack_queue_sender, ack_queue_receiever) = mpsc::channel::<WritePacket>(WRITE_PACKET_BUFFER_LEN);
        let (packet_sender, packet_receiver) = mpsc::channel::<WritePacket>(WRITE_PACKET_BUFFER_LEN);

        let ack_listener_handle = Self::listen_for_acks(reader, ack_queue_receiever, handle);
        let packet_sender_handle = Self::start_packet_sender(writer, packet_receiver, ack_queue_sender, handle);
    ...
    async fn shutdown(mut self) -> Result<WriteStatus> {
        ...
        let ack_status = self
            .ack_listener_handle
            .as_mut()
            .expect("ack listener is present until shutdown completes")
            .await
            .unwrap()?;
        self.ack_listener_handle.take();
        let (failed_nodes, unacked_packets) = match ack_status {
            WriteStatus::Success => {
                let packets_to_send = self
                    .packet_sender_handle
                    .as_mut()
                    .expect("packet sender is present until shutdown completes")
                    .await
                    .unwrap();
                self.packet_sender_handle.take();
                if !packets_to_send.is_empty() {
                    return Err(HdfsError::DataTransferError("Failed to send all packets to DataNode".to_string()));
                }
    ...
    fn start_packet_sender(
        ...
        handle.spawn(async move {
            while let Some(mut packet) = packet_receiver.recv().await {
                ...
                if crate::test::WRITE_CONNECTION_FAULT_INJECTOR.swap(false, std::sync::atomic::Ordering::SeqCst) {
                    debug!("Failing write to active node");
                    return [packet].into_iter().chain(Self::drain_queue(packet_receiver).await).collect();
                }
                ...
                if let Err(err) = ack_queue.send(packet).await {
                    ...
                    return [err.0].into_iter().chain(Self::drain_queue(packet_receiver).await).collect();
                };
    ...
    fn start_heartbeat_sender(packet_sender: mpsc::Sender<WritePacket>, handle: &Handle) -> JoinHandle<()> {
        handle.spawn(async move {
            loop {
            ...
            loop {
                let next_ack = match reader.read_ack().await {
                    ...
                    Err(e) => {
                        ...
                        return Ok(WriteStatus::Recover(vec![0], Self::drain_queue(ack_queue).await));
                    }
                };
                ...
                for (i, reply) in next_ack.reply().enumerate() {
                    ...
                    #[cfg(feature = "integration-test")]
                    if crate::test::WRITE_REPLY_FAULT_INJECTOR.lock().unwrap().take().is_some_and(|j| i == j) {
                ...
                if !failed_nodes.is_empty() {
                    ...
                    return Ok(WriteStatus::Recover(failed_nodes, Self::drain_queue(ack_queue).await));
                }
                ...
                if next_ack.seqno == UNKNOWN_SEQNO {
                    return Err(HdfsError::DataTransferError("Received unknown seqno for successful ack".to_string()));
                }
                ...
                if let Some(packet) = ack_queue.recv().await {
                    ...
                    if next_ack.seqno != packet.header.seqno {
                        return Err(HdfsError::DataTransferError(
                            "Received acknowledgement does not match expected sequence number".to_string(),
                        ));
                    }
...
impl Drop for Pipeline {
    fn drop(&mut self) {
        if let Some(handle) = &self.ack_listener_handle {
            handle.abort();
        }
        if let Some(handle) = &self.packet_sender_handle {
            handle.abort();
        }
...
mod pipeline_drop_tests {
    ...
    fn pending_pipeline(started: Arc<AtomicUsize>, dropped: Arc<AtomicUsize>) -> Pipeline {
        let (packet_sender, _packet_receiver) = mpsc::channel(1);
        let ack_started = started.clone();
        let ack_dropped = dropped.clone();
        let ack_listener_handle = tokio::spawn(async move {
            let _marker = DropMarker(ack_dropped);
            ack_started.fetch_add(1, Ordering::SeqCst);
            pending::<()>().await;
            Ok(WriteStatus::Success)
        });
        let sender_started = started.clone();
        let sender_dropped = dropped.clone();
        let packet_sender_handle = tokio::spawn(async move {
            let _marker = DropMarker(sender_dropped);
            sender_started.fetch_add(1, Ordering::SeqCst);
            pending::<()>().await;
            Vec::new()
        });
    ...
    #[tokio::test]
    async fn dropping_pipeline_aborts_all_background_tasks() {
        let started = Arc::new(AtomicUsize::new(0));
        let dropped = Arc::new(AtomicUsize::new(0));
        let pipeline = pending_pipeline(started.clone(), dropped.clone());
        wait_for_count(&started, 3).await;
        drop(pipeline);
        wait_for_count(&dropped, 3).await;
    }

    #[tokio::test]
    async fn cancelling_shutdown_aborts_handles_still_awaited_by_pipeline() {
        let started = Arc::new(AtomicUsize::new(0));
        let dropped = Arc::new(AtomicUsize::new(0));
        let pipeline = pending_pipeline(started.clone(), dropped.clone());
        wait_for_count(&started, 3).await;
...
impl ReplicatedBlockWriter {
    async fn new(
        ...
        let pipeline = Self::setup_pipeline(&protocol, &block, &server_defaults, None, None, &config, &handle).await?;
    ...
    async fn recover(
        ...
        debug!("Failed nodes: {:?}, block locs: {:?}", failed_nodes, self.block.locs);
        if failed_nodes.len() >= self.block.locs.len() {
            return Err(HdfsError::DataTransferError("All nodes failed for write".to_string()));
        }
        ...
        let updated_block = self.protocol.update_block_for_pipeline(self.block.b.clone()).await?;
        ...
        for packet in packets_to_replay {
            pipeline
                .send_packet(packet)
                .await
                .map_err(|_| HdfsError::DataTransferError("Failed to replay packets during recovery".to_string()))?;
        }
    ...
    async fn setup_pipeline(
        ...
        let stage = if new_gs.is_some() {
            hdfs::op_write_block_proto::BlockConstructionStage::PipelineSetupStreamingRecovery as i32
    ...
    async fn send_current_packet(&mut self) -> Result<()> {
        ...
        loop {
            let pipeline =
                self.pipeline.take().ok_or(HdfsError::DataTransferError("Block writer is closed".to_string()))?;
            ...
            if let Err(packet) = pipeline.send_packet(current_packet).await {
                ...
                match pipeline.shutdown().await? {
                    ...
                    WriteStatus::Recover(failed_nodes, packets_to_replay) => {
                        self.recover(failed_nodes, packets_to_replay, Some(&current_packet)).await?;
                    }
                }
            } else {
                self.pipeline = Some(pipeline);
    ...
    async fn write(&mut self, buf: &mut Bytes) -> Result<()> {
        ...
        let bytes_to_write = usize::min(buf.len(), self.block_size - self.block.b.num_bytes() as usize);
        ...
        while !buf_to_write.is_empty() {
            ...
            *self.block.b.num_bytes.as_mut().unwrap() += (initial_buf_len - buf_to_write.len()) as u64;
    ...
    async fn close(mut self) -> Result<hdfs::ExtendedBlockProto> {
        ...
        loop {
            match self
                .pipeline
                .take()
                .ok_or(HdfsError::DataTransferError("Block writer closed prematurely".to_string()))?
                .shutdown()
                .await?
            {
    ...
    async fn transfer_block(
        ...
        if response.status != hdfs::Status::Success as i32 {
            return Err(HdfsError::DataTransferError("Failed to add a datanode".to_string()));
        }
    ...
    async fn add_datanode_to_pipeline(
        ...
        let new_node_idx = new_nodes
            ...
            .position(|node| {
                !original_nodes
                    .iter()
                    .any(|orig| orig.id.ip_addr == node.id.ip_addr && orig.id.xfer_port == node.id.xfer_port)
            })
            .ok_or_else(|| {
                HdfsError::DataTransferError("No new datanode found in updated block locations".to_string())
            })?;

        let src_node = if new_node_idx == 0 { new_nodes[1].clone() } else { new_nodes[new_node_idx - 1].clone() };
...
impl CellBuffer {
    fn new(ec_schema: &EcSchema) -> Self {
        let buffers = (0..ec_schema.data_units).map(|_| BytesMut::with_capacity(ec_schema.cell_size)).collect();
        Self {
            buffers,
...
impl StripedBlockWriter {
    ...
    async fn write_cells(&mut self) -> Result<()> {
        ...
        for (index, (data, writer)) in
            self.cell_buffer.encode().into_iter().zip(self.block_writers.iter_mut()).enumerate()
        {
        ...
        let failed_writers =
            self.block_writers.iter().filter(|writer| writer.as_ref().is_some_and(|w| w.is_err())).count();
        if failed_writers > (self.block_writers.len() - self.data_units) {
            return Err(HdfsError::DataTransferError("Insufficient blocks succeeded during write".to_string()));
        }
