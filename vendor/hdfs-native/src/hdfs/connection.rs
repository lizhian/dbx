...
use crate::security::sasl::{SaslDatanodeConnection, SaslDatanodeReader, SaslDatanodeWriter, negotiate_sasl_session};
...
pub(crate) static DATANODE_CACHE: Lazy<DatanodeConnectionCache> = Lazy::new(DatanodeConnectionCache::new);

struct AbortOnDropJoinHandle<T>(Option<JoinHandle<T>>);
...
impl<T> AbortOnDropJoinHandle<T> {
    ...
    async fn join(mut self) -> std::result::Result<T, tokio::task::JoinError> {
        let result = self.0.as_mut().expect("task handle is present until join completes").await;
        self.0.take();
        result
    }
}
...
async fn connect(addr: &str, handle: &Handle) -> Result<TcpStream> {
    ...
    // Preserve the configured I/O runtime while retaining the handle until
    // completion, so cancellation aborts the spawned connect task.
    let stream = AbortOnDropJoinHandle::new(handle.spawn(TcpStream::connect(addr))).join().await.unwrap()?;
...
impl AlignmentContext {
    fn update(&mut self, state_id: Option<i64>, router_federated_state: Option<Vec<u8>>) -> Result<()> {
        ...
        if let Some(new_router_state) = router_federated_state {
            let new_map = hdfs::RouterFederatedStateProto::decode(Bytes::from(new_router_state))?.namespace_state_ids;
            ...
            for (key, value) in new_map.into_iter() {
                current_map.insert(key.clone(), i64::max(value, *current_map.get(&key).unwrap_or(&i64::MIN)));
            }
        }
    ...
    fn encode_router_state(&self) -> Option<Vec<u8>> {
        self.router_federated_state
            .as_ref()
            .map(|state| hdfs::RouterFederatedStateProto { namespace_state_ids: state.clone() }.encode_to_vec())
    }
}

impl Default for AlignmentContext {
    fn default() -> Self {
        Self { state_id: i64::MIN, router_federated_state: None }
    }
}
...
impl RpcConnection {
    pub(crate) async fn connect(
        ...
        let service = nameservice.map(|ns| format!("ha-hdfs:{ns}")).unwrap_or(url.to_string());
        ...
        conn.sender_handle = Some(conn.start_sender(receiver, writer, handle));

        let context_header = conn.get_connection_header(-3, -1).encode_length_delimited_to_vec();
        let context_msg = conn.get_connection_context().encode_length_delimited_to_vec();
        conn.write_messages(&[&context_header, &context_msg]).await?;
    ...
    fn start_listener(&mut self, reader: SaslReader, handle: &Handle) -> Result<JoinHandle<()>> {
        ...
        let listener = handle.spawn(async move {
            RpcListener::new(call_map, reader, alignment_context).start().await;
        });
        Ok(listener)
    }
    ...
    fn get_connection_header(&self, call_id: i32, retry_count: i32) -> common::RpcRequestHeaderProto {
        let (state_id, router_federated_state) = if let Some(context) = self.alignment_context.as_ref() {
    ...
    fn get_connection_context(&self) -> common::IpcConnectionContextProto {
        ...
        let context =
            common::IpcConnectionContextProto { protocol: Some(PROTOCOL.to_string()), user_info: Some(user_info) };
    ...
    pub(crate) async fn call(&self, method_name: &str, message: &[u8]) -> Result<oneshot::Receiver<Result<Bytes>>> {
        ...
        self.write_messages(&[&conn_header_buf, &header_buf, message]).await?;
...
impl Drop for RpcConnection {
    fn drop(&mut self) {
        abort_rpc_connection_tasks(&mut self.listener, &mut self.sender_handle, &self.call_map);
    }
}

fn abort_rpc_connection_tasks(
    listener: &mut Option<JoinHandle<()>>,
    sender: &mut Option<JoinHandle<()>>,
    call_map: &Mutex<Option<HashMap<i32, CallResult>>>,
) {
...
impl RpcListener {
    fn new(
        ...
        RpcListener { call_map, reader, alive: true, alignment_context }
    }
    ...
    async fn read_response(&mut self) -> Result<()> {
        ...
        let call = self.call_map.lock().unwrap().as_mut().and_then(|m| m.remove(&call_id));
        ...
        if let Some(call) = call {
            match rpc_response.status() {
                ...
                RpcStatusProto::Fatal => {
                    warn!("RPC fatal error: {}: {}", rpc_response.exception_class_name(), rpc_response.error_msg());
...
impl ReadPacket {
    ...
    pub(crate) fn get_data(mut self, checksum_info: &Option<hdfs::ReadOpChecksumInfoProto>) -> Result<Bytes> {
        ...
        if let Some(info) = checksum_info {
            ...
            if let Some(algorithm) = algorithm {
                ...
                while !checksum_data.is_empty() {
                    let chunk_checksum = algorithm.checksum(
                        &checksum_data
                            .split_to(usize::min(info.checksum.bytes_per_checksum as usize, checksum_data.len())),
                    );
...
impl WritePacket {
    pub(crate) fn empty(offset: i64, seqno: i64, bytes_per_checksum: u32, max_packet_size: u32) -> Self {
        let header = hdfs::PacketHeaderProto { offset_in_block: offset, seqno, ..Default::default() };
    ...
    pub(crate) fn write(&mut self, buf: &mut Bytes) {
        self.data.put(buf.split_to(usize::min(self.max_data_size - self.data.len(), buf.len())));
...
impl DatanodeConnection {
    pub(crate) async fn connect(
        ...
        let address = if config.datanode_hostname_enabled() && !datanode_id.host_name.is_empty() {
            &datanode_id.host_name
        } else {
            &datanode_id.ip_addr
        };
        let url = format!("{}:{}", address, datanode_id.xfer_port);
        let stream = connect(&url, handle).await?;

        let sasl_connection = SaslDatanodeConnection::create(stream);
        let (reader, writer) = sasl_connection.negotiate(datanode_id, token, encryption_key.as_ref(), config).await?;

        let conn = DatanodeConnection { client_name: Uuid::new_v4().to_string(), reader, writer };
        Ok(conn)
    }

    pub(crate) async fn send(&mut self, op: Op, message: &impl Message) -> Result<hdfs::BlockOpResponseProto> {
        self.writer.write_all(&DATA_TRANSFER_VERSION.to_be_bytes()).await?;
        self.writer.write_all(&[op.value()]).await?;
        self.writer.write_all(&message.encode_length_delimited_to_vec()).await?;
    ...
    pub(crate) fn build_header(
        ...
        let base_header = hdfs::BaseHeaderProto { block: block.clone(), token, ..Default::default() };

        hdfs::ClientOperationHeaderProto { base_header, client_name: self.client_name.clone() }
    }
    ...
    pub(crate) async fn read_packet(&mut self) -> Result<ReadPacket> {
        ...
        let header = hdfs::PacketHeaderProto::decode(remaining_buf.split_to(header_length).freeze())?;
    ...
    pub(crate) async fn send_read_success(&mut self) -> Result<()> {
        let client_read_status = hdfs::ClientReadStatusProto { status: hdfs::Status::ChecksumOk as i32 };

        self.writer.write_all(&client_read_status.encode_length_delimited_to_vec()).await?;
        self.writer.flush().await?;
    ...
    pub(crate) fn split(self) -> (DatanodeReader, DatanodeWriter) {
        let reader = DatanodeReader { reader: self.reader };
        let writer = DatanodeWriter { writer: self.writer };
        (reader, writer)
    }
}
...
impl DatanodeWriter {
    ...
    pub(crate) async fn write_packet(&mut self, packet: &mut WritePacket) -> Result<()> {
        ...
        self.writer.write_all(&(header_encoded.len() as u16).to_be_bytes()).await?;
...
impl DatanodeConnectionCache {
    ...
    pub(crate) fn get(&self, _datanode_id: &hdfs::DatanodeIdProto) -> Option<DatanodeConnection> {
        None
    }

    pub(crate) fn release(&self, _conn: DatanodeConnection) {
        // DBX disables the process-global cache: it is not scoped by DBX
        // connection or identity and only expires lazily on a later read.
    }
}

#[cfg(test)]
mod connect_task_drop_tests {
    use std::{
        future::pending,
        sync::{
            Arc,
    ...
    #[tokio::test]
    async fn cancelling_join_aborts_spawned_connect_task() {
        let dropped = Arc::new(AtomicBool::new(false));
        let task_dropped = dropped.clone();
        let task = AbortOnDropJoinHandle::new(tokio::spawn(async move {
        ...
        tokio::task::yield_now().await;
        {
            ...
            tokio::pin!(join);
            tokio::select! {
                _ = &mut join => panic!("pending connect task unexpectedly completed"),
...
mod rpc_connection_drop_tests {
    ...
    #[tokio::test]
    async fn aborts_both_tasks_and_fails_pending_calls() {
        let dropped = Arc::new(AtomicUsize::new(0));
        let listener_dropped = dropped.clone();
        let sender_dropped = dropped.clone();
        let mut listener = Some(tokio::spawn(async move {
            let _marker = DropMarker(listener_dropped);
            pending::<()>().await;
        }));
        let mut sender = Some(tokio::spawn(async move {
            let _marker = DropMarker(sender_dropped);
            pending::<()>().await;
        }));
        tokio::task::yield_now().await;
        let (call_sender, call_receiver) = oneshot::channel();
        let call_map = Mutex::new(Some(HashMap::from([(7, call_sender)])));

        abort_rpc_connection_tasks(&mut listener, &mut sender, &call_map);

        let error = call_receiver.await.unwrap().unwrap_err();
        assert!(matches!(error, HdfsError::IOError(_)));
        tokio::time::timeout(std::time::Duration::from_secs(1), async {
...
#[cfg(test)]
mod datanode_cache_contract_tests {
    use super::*;

    #[test]
    fn cache_is_stateless_and_never_returns_cross_connection_sockets() {
        assert_eq!(std::mem::size_of::<DatanodeConnectionCache>(), 0);
        assert!(DATANODE_CACHE.get(&DatanodeIdProto::default()).is_none());
    }
}
...
mod test {
    ...
    fn test_max_packet_header_size() {
        ...
        let header = hdfs::PacketHeaderProto { sync_block: Some(false), ..Default::default() };
    ...
    fn encode_router_state(map: &HashMap<String, i64>) -> Vec<u8> {
        hdfs::RouterFederatedStateProto { namespace_state_ids: map.clone() }.encode_to_vec()
    }
    ...
    fn test_router_federated_state() {
        ...
        alignment_context.update(None, Some(encode_router_state(&state_map))).unwrap();
